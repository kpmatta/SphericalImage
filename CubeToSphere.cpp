...
/** \brief The initialise function of CubeToSphere. This one takes fewer parameters, and will
...
*
*/
void CubeToSphere::init(const unsigned int pxInW, const double rdInV, const double rdInH)
{
...
/** \brief The initialise function of CubeToSphere.
*
...
*
*/
void CubeToSphere::init(const unsigned int pxPanoH, const unsigned int pxPanoV, const unsigned int pxInW, const double rdInV, const double rdInH) 
{
	// check parameters
	if ((pxInW == 0)
		|| (pxPanoH == 0)
...
* \return CUBE_COORD
*
const CUBE_COORD* CubeToSphere::getCoord(const unsigned int x, const unsigned int y) const {
...
*
*/
inline void CubeToSphere::rotRad(double rad, double& x, double& y, double& temp) 
{
	temp = x;
...
*
*/
inline void CubeToSphere::transDis(double dis, double& x, double& y) 
{
	x += dis;
	y += dis;
}
...
*
*/
inline void CubeToSphere::calXY(const int& i, const int& j) 
{
	calXYZ(i, j, tX, tY, tZ);
...
*
*/
inline void CubeToSphere::locate(const double& axis, const double& px, const double& py, const double& rad) 
{
...
*
*/
inline void CubeToSphere::calCubeFace(const double& theta, const double& phi) 
{
...
*
*/
inline void CubeToSphere::calXYZ(const int& i, const int& j, double& x, double& y, double& z) 
{
	calNormXY(i, j, x, y);
...
inline void CubeToSphere::calNormXY(const int& i, const int& j, double& x, double& y) 
{
...
inline void CubeToSphere::calThetaAndPhi(const double& x, const double& y, double& theta, double& phi) 
{
	theta = x * M_PI;
	phi = y * M_PI_2;
...
inline void CubeToSphere::calXyzFromThetaPhi(const double& theta, const double& phi, double& x, double& y, double& z) 
{
	x = cos(phi) * cos(theta);
	y = sin(phi);
	z = cos(phi) * sin(theta);
...
*
*/
inline bool CubeToSphere::cmpDoubleEqual(const double& a, const double& b, const double& epsilon)
{
...
*
*/
inline bool CubeToSphere::cmpDoubleSmaller(const double& a, const double& b, const double& epsilon)
{
...
*
*/
inline bool CubeToSphere::cmpDoubleEqualSmaller(const double& a, const double& b, const double& epsilon) 
{
	return ((a - b) < 0) || CubeToSphere::cmpDoubleEqual(a, b, epsilon);
}
...
*
*/
inline bool CubeToSphere::cmpDoubleLarger(const double& a, const double& b, const double& epsilon) {
	return ((a - b) > 0) && (!cmpDoubleEqual(a, b, epsilon));
}
...
*
*/
inline bool CubeToSphere::cmpDoubleEqualLarger(const double& a, const double& b, const double& epsilon) {
	return ((a - b) > 0) || CubeToSphere::cmpDoubleEqual(a, b, epsilon);
}
...
*
*/
inline bool CubeToSphere::isDoubleInRange(const double& value, const double& dblSmall, const double& large, const double& epsilon) {
	return    cmpDoubleEqualLarger(value, dblSmall, epsilon)
...
*
*/
CubeToSphere::CubeToSphere(void)
	: pxCamV(0),
	pxCamH(0),
	radius(0.0),
	rdPanoV(0.0),
